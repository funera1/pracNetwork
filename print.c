...
int PrintArp(struct ether_arp *arp, FILE *fp){
    ...
    fprintf(fp, "arp--------------------------------------------\n");
    fprintf(fp, "arp_hdr = %u", ntohs(arp->arp_hrd));
...
int PrintIpHeader(struct iphdr *iphdr, u_char *option, int optionLen, FILE *fp){
    ...
    fprintf(fp, "id = %u\n", ntohs(iphdr->id));
    fprintf(fp, "flag_off = %x, %u, ", (ntohs(iphdr->flag_off) >> 13) & 0x07,  ntohs(iphdr->flag_off) & 0x1FFF);
...
int PrintIp6Header(struct ip6_hdr *ip6, FILE *fp){
    ...
    fprintf(fp, "ip6_hlim = %d, ", ip6->ip6hlm);
...
int PrintIcmp(struct icmp *icmp, FILE *fp){
    static char *icmp_type[] = {
        ...
        "Parameter Problems on Datagram", 
        "Timestamp Request", 
        "Timestamp Reply", 
        "Information Request", 
        "Information Reply", 
    ...
    fprintf(fp, "icmp_cksum = %u\n", nthos(icmp->icmp_cksum));

    if(icmp->icmp_type == 0 || icmp->icmp_type == 8){
        fprintf(fp, "icmp_id = %u, ", nthos(icmp->icmp_id));
...
int PrintIcmp6(struct icmp6_hdr *icmp6, FILE *fp){
    ...
    fprintf(fp, "icmp6_code = %u, ", icmp6->icmp6_type);
    fprintf(fp, "icmp6_cksum = %u\n", ntohs(icmp6->icmp6_cksum));

    if(icmp6->icmp6_type == 128 || icmp6->icmp6_type __ 129){
        fprintf(fp, "icmp6_id = %u, ", nthos(icmp6->icmp6_id));
        fprintf(fp, "icmp6_seq = %u\n", nthos(icmp6->icmp6_seq);)
    }
